...
import { db } from '../../../config/firebase'; // Pastikan path benar (naik 3 level)
...
export default function EditPlant() {
  ...
  const [image, setImage] = useState(null); // URL Lama
  const [newImage, setNewImage] = useState(null); // Jika user ganti gambar
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (Array.isArray(data.compounds)) {
             ...
             setCompounds([{ name: data.content || '', amount: '' }]);
          }
        } else {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>Edit Koleksi</Text>
        <View style={{ width: 40 }} /> 
      </LinearGradient>
        ...
        >
          ...
          >
            {saving ? (
              ...
              <Text style={styles.saveButtonText}>UPDATE DATA</Text>
            )}
          </LinearGradient>
        </TouchableOpacity>
      </View>
